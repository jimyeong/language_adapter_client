...
const MyEventsDetailContainerBlock = styled.div``;
const bottomButtonFooterSize = 46;
const backbuttonTopPaddingSize = backButtonSpace[0];

/* 종일플래그가 있으니 확인해볼것 */
const MyEventsDetailContainer = ({ children, ...rest }) => {
    ...
    const initialStatusSwitches = {
        pushSwitch: state.infoAlarm === 'Y' ? '1' : '0',
        AllDaySwitch: state.infoAlways === 'Y' ? '1' : '0',
    };
    const calendarSchedule = {
        startDay: new Date(state.infoStartDt), // 시작날짜 초기값 오늘
        endDay: new Date(state.infoEndDt), // 종료날짜 초기값 내일
    };
    ...
    const [switchStatus, setSwitchStatus] = useState(
        initialStatusSwitches.pushSwitch
    ); // 푸시알림 스위치
    const [currentColor, setCurrentColor] = useState(state.infoColor); // 현재 선택된 색상
    const [isAllDay, setIsAllDay] = useState(
        initialStatusSwitches.AllDaySwitch
    ); // 종일설정
    ...
    const [_inputValues, onChangeInputs, onResetInputs] = useInputs({
        personalMemo: state.infoMemo,
        eventTitle: state.infoSubject,
    });
    ...
    const deleteApiCall = async () => {
        ...
        const successMessage = '일정 삭제되었습니다';
    ...
    return (
        <MyEventsDetailContainerBlock>
            <MyEventDetail
                ...
                isAllDay={isAllDay}
                onChangeAllDay={(status) => setIsAllDay(status)}
...
export default MyEventsDetailContainer;
